...
class SyncJobRepository(BaseRepository[SyncJob]):
    ...
    async def get_jobs_paginated(
        ...
        query = select(SyncJob).order_by(desc(SyncJob.created_at))
        if source_slug:
            query = query.where(SyncJob.source_slug == source_slug)
    ...
    async def count_jobs(
        ...
        """Count total sync jobs."""
        query = select(func.count()).select_from(SyncJob)
        if source_slug:
            query = query.where(SyncJob.source_slug == source_slug)
        result = await db.execute(query)
...
def get_sync_job_repository() -> SyncJobRepository:
    """Dependency injection for SyncJobRepository."""
